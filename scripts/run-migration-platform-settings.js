#!/usr/bin/env node
/**
 * ينفّذ shared/migration_platform_settings.sql على قاعدة Supabase.
 *
 * مطلوب أحد الخيارات في .env:
 *   SUPABASE_DB_URL=postgresql://postgres:[PASSWORD]@db.[ref].supabase.co:5432/postgres
 *   أو DATABASE_URL
 *   أو SUPABASE_DB_PASSWORD (مع SUPABASE_URL)
 *
 * على Windows مع مشاكل SSL:
 *   set NODE_OPTIONS=--use-system-ca
 *
 * بديل: Supabase Dashboard → SQL Editor → الصق محتوى الملف يدوياً.
 */
const fs = require("fs");
...
async function verifyWithSupabase() {
  ...
  const { error } = await sb.from("platform_settings").select("key").limit(1);
  if (error) {
    if (/platform_settings|42P01|schema cache/i.test(String(error.message || ""))) {
...
async function main() {
  const sqlPath = path.join(__dirname, "..", "shared", "migration_platform_settings.sql");
  ...
  if (!dbUrl) {
    console.error(
      [
        ...
        "",
        "— أو انسخ الملف يدوياً في SQL Editor:",
        "  " + sqlPath,
      ].join("\n")
    );
    process.exit(1);
  ...
  console.log("[migrate] تنفيذ migration_platform_settings.sql …");
  ...
  if (!v.ok) {
    ...
    console.warn("قد يحتاج PostgREST بضع ثوانٍ لتحديث schema cache — أعد المحاولة أو أعد تشغيل الخادم.");
    process.exit(2);
  }

  console.log("[migrate] جدول platform_settings جاهز.");
}
