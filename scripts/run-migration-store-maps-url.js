#!/usr/bin/env node
/**
 * ينفّذ shared/migration_store_maps_url.sql ثم يعيد تحميل PostgREST schema cache.
 */
const fs = require("fs");
...
async function verifyWithSupabase() {
  ...
  const { error } = await sb.from("stores").select("id, maps_url").limit(1);
...
async function main() {
  ...
  const sqlPath = path.join(__dirname, "..", "shared", "migration_store_maps_url.sql");
  if (!dbUrl) {
    console.error("أضِف SUPABASE_DB_URL أو SUPABASE_DB_PASSWORD إلى .env ثم أعد التشغيل.");
    process.exit(1);
  }
  const sql = fs.readFileSync(sqlPath, "utf8");
  console.log("[migrate] تنفيذ migration_store_maps_url.sql …");
  ...
  console.log("[migrate] stores.maps_url جاهز وschema cache محدّث.");
}
