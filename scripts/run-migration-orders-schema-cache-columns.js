#!/usr/bin/env node
/**
 * ينفّذ shared/migration_orders_schema_cache_columns.sql على Supabase Postgres.
 */
const fs = require("fs");
...
async function main() {
  ...
  const sql = fs.readFileSync(
    path.join(__dirname, "..", "shared", "migration_orders_schema_cache_columns.sql"),
    "utf8"
  );
  await runWithPg(dbUrl, sql);
  await runWithPg(dbUrl, "NOTIFY pgrst, 'reload schema';");
  console.log("[migrate] orders schema cache columns (breakdown, data, payment_*, store_*) جاهزة.");
}
