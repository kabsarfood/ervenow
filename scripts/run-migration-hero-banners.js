#!/usr/bin/env node
/**
 * ينفّذ shared/migration_hero_banners.sql على قاعدة Supabase.
 *
 ...
 *
 * بديل: Supabase Dashboard → SQL Editor → الصق محتوى shared/migration_hero_banners.sql
 */
const fs = require("fs");
...
async function verifyWithSupabase() {
  ...
  const { error } = await sb.from("hero_banners").select("id").limit(1);
  if (error) {
    if (/hero_banners|42P01|schema cache/i.test(String(error.message || ""))) {
...
async function main() {
  const sqlPath = path.join(__dirname, "..", "shared", "migration_hero_banners.sql");
  ...
  console.log("[migrate] تنفيذ migration_hero_banners.sql …");
  ...
  console.log("[migrate] جدول hero_banners جاهز.");
}
