#!/usr/bin/env node
/**
 * ينفّذ shared/migration_users_service_district.sql (+ vehicle إن وُجد)
 * على قاعدة Supabase.
 *
 * مطلوب في .env:
...
async function verifyWithSupabase() {
  ...
  const { error } = await sb.from("users").select("id, service_district").limit(1);
  if (error) {
    if (/service_district/i.test(String(error.message || ""))) {
...
async function main() {
  ...
  if (!dbUrl) {
    console.error(
      [
        ...
        "",
        "ثم:",
        "  npm run migrate:users-service-district",
      ].join("\n")
    );
    process.exit(1);
  }
  ...
  const files = [
    "migration_users_service_district.sql",
    "migration_users_service_vehicle.sql",
  ];

  for (const file of files) {
    console.log(`[migrate] تنفيذ ${file} …`);
    await runWithPg(dbUrl, readSql(file));
  }
  ...
  console.log("[migrate] عمود users.service_district جاهز.");
}
