#!/usr/bin/env node
/**
 * ينفّذ shared/migration_hotfix_003_wallet_balance_from_tx.sql
 */
const fs = require("fs");
...
async function main() {
  ...
  const sql = fs.readFileSync(
    path.join(__dirname, "..", "shared", "migration_hotfix_003_wallet_balance_from_tx.sql"),
    "utf8"
  );
  await runWithPg(dbUrl, sql);
  await runWithPg(dbUrl, "NOTIFY pgrst, 'reload schema';");
  console.log("[migrate] wallet balance from transactions + platform wallet seed جاهز.");
}
