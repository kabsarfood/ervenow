/**
 * Run notifications table migration on Supabase.
 * Usage: node scripts/run-migration-notifications.js
 */
const fs = require("fs");
...
async function main() {
  ...
  const sqlPath = path.join(__dirname, "..", "shared", "migration_notifications.sql");
  ...
  try {
    ...
    console.log("[migrate] migration_notifications applied");
  } finally {
    await client.end();
  }
}
