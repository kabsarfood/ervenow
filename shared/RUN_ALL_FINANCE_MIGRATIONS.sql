...
BEGIN
  ...
  -- HOTFIX-001: لا إيداع تاجر من RPC — Node (storeMerchantLedgerCredit) بمرجع order:{id}:merchant_net
  amt_merchant := 0;
...
-- ─── migration_ervenow_ledger_withdraw_requests.sql (دوال فقط — بدون إعادة إنشاء الجداول) ───
...
-- تحقق سريع:
SELECT 'ervenow_ledger_wallets' AS tbl, count(*)::int AS n FROM public.ervenow_ledger_wallets
UNION ALL SELECT 'ervenow_ledger_transactions', count(*)::int FROM public.ervenow_ledger_transactions
UNION ALL SELECT 'settlement_log', count(*)::int FROM public.settlement_log
UNION ALL SELECT 'withdraw_requests', count(*)::int FROM public.withdraw_requests;